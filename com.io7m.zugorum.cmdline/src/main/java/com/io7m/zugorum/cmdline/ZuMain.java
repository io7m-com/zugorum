...
package com.io7m.zugorum.cmdline;
...
import com.io7m.zugorum.cmdline.internal.ZuCmdServer;
...
public final class ZuMain implements Runnable
{
  private static final Logger LOG =
    LoggerFactory.getLogger(ZuMain.class);
    ...
    final String[] inArgs)
  {
    ...
    final var metadata =
      new QApplicationMetadata(
        ...
        "The zugorum liveness checker.",
        Optional.of(URI.create("https://www.io7m.com/software/zugorum/"))
      );

    final var builder = QApplication.builder(metadata);
    builder.allowAtSyntax(true);
    builder.addCommand(new ZuCmdServer());
  ...
  public static int mainExitless(
    final String[] args)
  {
    final ZuMain cm = new ZuMain(args);
    cm.run();
    return cm.exitCode();
  }
